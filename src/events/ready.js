module.exports = async (client) => {
  ...
  const activities = [
    { name: 'Master', type: 'LISTENING' }, 
    { name: 'Daddy rangy', type: 'LISTENING' }
  ];
  ...
  setInterval(() => {
    activities[2] = { name: `Ranger jack off`, type: 'WATCHING' }; // Update server count
    activities[3] = { name: `Ranger moan`, type: 'LISTENING' }; // Update user count
  ...
  for (const guild of leftGuilds) {
    ...
    client.logger.info(`Weewoo has left ${guild.guild_name}`);
  }

  client.logger.info('Weewoo is now online');
  client.logger.info(`Weewoo is running on ${client.guilds.cache.size} server(s)`);
};
